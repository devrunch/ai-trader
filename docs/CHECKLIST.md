...
- 2026-09-13 · — · Gold charts loaded but 1m still failed *through the API*, which the signals-level checks never touched: a cold fetch took 18 s against a 10 s upstream timeout, so 1m 503'd while 15m and 1d scraped through. Four causes, all measured on the box: a new WebSocket per page (1.48 s of handshake for a 50 ms answer) where Deriv correlates responses by `req_id` and one connection serves all of them; windows fetched one after another though each carries its own `end`; `dukascopy-node` pausing a full second between batches of ten hourly files; and the bridge shipping millions of tick timestamps through a pipe for Python to bucket. Cold 1m/5d 18 s → 8.3 s, 1d 7.7 → 3.2 s, tick volume 5.0 → 1.3 s, eight pages 13.8 → 1.0 s. The API's bars budget went to 25 s so a cold multi-window fetch has room. · `ai-trader-signals@HEAD`, `ai-trader-api@HEAD`
